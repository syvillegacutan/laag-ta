...
function WebMap({ lat, lng, focusLat, focusLng }: { lat: number; lng: number; focusLat?: number; focusLng?: number }) {
  const centerLat = focusLat ?? lat;
  const centerLng = focusLng ?? lng;
  const delta = focusLat ? 0.004 : 0.008;
  ...
  return (
    <View style={styles.mapContainer}>
      {/* @ts-ignore - iframe is valid in web context */}
    ...
    return (
      <View style={styles.centered}>
        ...
        <Text style={styles.permissionText}>
          Laag Ta! needs your location to find nearby places. Please enable it in your browser settings.
        </Text>
  ...
  return (
    <View style={styles.container}>
      <WebMap
        lat={location.coords.latitude}
        lng={location.coords.longitude}
        focusLat={selectedPlace?.lat}
        focusLng={selectedPlace?.lng}
      />
